...
class TestViewer(TestCase):
    def setUp(self) -> None:
        super().setUp()
        self._t1 = np2torch(med_load("subject-9-T1.hdr")[0])
        self._t2 = np2torch(med_load("subject-9-T2.hdr")[0])
        self._l1 = np2torch(med_load("subject-9-label.hdr")[0] == 10)
        self._l2 = np2torch(med_load("subject-9-label.hdr")[0] == 150)
        self._l3 = np2torch(med_load("subject-9-label.hdr")[0] == 250)
