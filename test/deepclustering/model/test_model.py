...
class TestModel(TestCase):
    def setUp(self) -> None:
        ...
        self.optim_dict = {"name": "Adam", "lr": 1e-4, "weight_decay": 1e-5}
        self.scheduler_dict = {
            "name": "MultiStepLR",
    ...
    def test_state_dict(self):
        for arch_name, arch_dict in self.arch_dicts.items():
            ...
            input_img = (
                self.image
                if arch_name != "clusternetimsat"
                else torch.randn(10, 1, 28, 28)
            )
            predicts = model1.predict(input_img)
            model2 = self._init_onenet(
                arch_dict=arch_dict,
                optim_dict=self.optim_dict,
                scheduler_dict=self.scheduler_dict,
            )
    ...
    def test_initialize_instance_from_cls_save(self):
        ...
        for arch_name, arch_dict in self.arch_dicts.items():
            ...
            state_dict = torch.load(
                f"{Path(__file__).parent / arch_name}.pth",
