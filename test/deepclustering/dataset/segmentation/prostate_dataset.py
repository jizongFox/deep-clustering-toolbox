...
class TestDownloadDataset(TestCase):
    ...
    def setUp(self) -> None:
        ...
        if Path(self.dataset_root, ProstateDataset.folder_name).exists():
            shutil.rmtree(Path(self.dataset_root, ProstateDataset.folder_name), ignore_errors=True)
        if Path(self.dataset_root, ProstateDataset.zip_name).exists():
            os.remove(Path(self.dataset_root, ProstateDataset.zip_name))

    def test_download_dataset(self):
        dataset = ProstateDataset(root_dir=self.dataset_root, subfolders=self.dataset_subfolders, verbose=True,
                              mode="train")
