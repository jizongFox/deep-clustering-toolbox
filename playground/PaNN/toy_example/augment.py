...
def inverse_transform_matrix(AffineMatrix: torch):
    def _inverse_transform_matrix(AffineMatrix: torch):
        ...
        fullMatrix = torch.cat(
            [AffineMatrix,
             torch.from_numpy(np.array([0, 0, 1]).reshape(1, 3).astype(np.float32)).to(AffineMatrix.device)],
            dim=0)
...
class AffineTensorTransform(object):
    ...
    def __init__(self, min_rot: float = 0, max_rot: float = 180, min_shear: float = 0.0,
                 max_shear: float = 0.5, min_scale: float = 0.7, max_scale: float = 1.3,
                 mode='bilinear'):
    ...
    def get_random_affinematrix(self):
        ...
        RandomAffineMatrix = np.array([[np.cos(a) * scale, - np.sin(a + shear) * scale, 0.],
                                       [np.sin(a) * scale, np.cos(a + shear) * scale, 0.]], dtype=np.float32)  # 3x3
    ...
    def _perform_affine_transform(self, single_img: Tensor, affinematrix: Tensor):
        ...
        grid = F.affine_grid(affinematrix, single_img.shape)  # output should be same size
        data_tf = F.grid_sample(single_img, grid, mode=self.mode,
                                padding_mode="zeros")  # this can ONLY do bilinear
        return data_tf

    def __call__(self, img: Tensor, AffineMatrix: Tensor = None, independent=True, inverse=False):
