...
parser = argparse.ArgumentParser(description='PyTorch Differentiable Discrete Augmented Training')
parser.add_argument('--lr', default=0.1, type=float, help='learning rate')
parser.add_argument('--dlr', default=0.1, type=float, help='data augmentation learning rate multiplier')
...
parser.add_argument('--gradit', '-g', type=int, default=1, help='Number of gradient itereations in the past')
parser.add_argument('--whengrad', type=int, default=1, help='How often update the gradient')
parser.add_argument('--factor', '-f', type=float, default=0.5, help='Initial value for factor')
parser.add_argument('--kchannels', '-k', type=int, default=1, help='Multiplier of used channels ')
parser.add_argument('--schedule1', type=int, default=200, help='Epoch to reduce schedule by 10, -1 to not use')
parser.add_argument('--smpuniform', type=int, default=-1,
                    help='Sample uniform for estimation of weights every X times and the rest is based on --sampling')
parser.add_argument('--schedule2', type=int, default=400, help='Second epoch to reduce schedule by 10, -1 to not use')
parser.add_argument('--sampling', type=float, default=0.5, help='Sampling ratio: 0=uniform 1=greedy(wont work)')
parser.add_argument('--stoch', '-s', action='store_true', help='Stochastic')
parser.add_argument('--sametr', action='store_true',
...
parser.add_argument('--learnCfactors', '-l', action='store_true', help='Learn Continuous Factors')
...
class Affine(object):
    ...
    def __call__(self, img):
        ...
        """
        return torchvision.transforms.functional.affine(img, self.angle, self.translate, self.scale, self.shear,
                                                        self.resample, self.fillcolor)
...
if args.dataset == 'MNIST':
    ...
    transform_valtest = transforms.Compose([
        # transforms.RandomCrop(isize, padding=isize/4),#8),
        # Affine(angle=90, translate=(0,0), scale=1, shear=0, resample=False, fillcolor=0),
        transforms.RandomAffine(degrees=(0, 0), translate=(0, 0), scale=(1, 1), shear=None, resample=False,
                                fillcolor=0),
        transforms.ToTensor(),
        transforms.Lambda(lambda x: torch.cat([x, x, x], 0)),
        transforms.Normalize((0.1307,), (0.3081,))
    ...
    trainset1 = torchvision.datasets.MNIST('./data', train=True, download=True, transform=transform_train)
    valtestset = torchvision.datasets.MNIST(root='./data', train=False, transform=transform_valtest)
    testset = torchvision.datasets.MNIST('./data', train=False, transform=transform_test)
elif args.dataset == 'CIFAR10':
    trainset1 = CIFAR10ID(root='./data', train=True, download=True, transform=transform_train)
    valtestset = CIFAR10ID(root='./data', train=False, download=True, transform=transform_valtest)
    testset = CIFAR10ID(root='./data', train=False, download=True, transform=transform_test)
elif args.dataset == 'STL10':
    trainset1 = torchvision.datasets.STL10(root='./data', split='test', download=True, transform=transform_train)
    valtestset = torchvision.datasets.STL10(root='./data', split='train', download=True, transform=transform_valtest)
    testset = torchvision.datasets.STL10(root='./data', split='train', download=True, transform=transform_test)
...
trainloader = torch.utils.data.DataLoader(trainset, batch_size=args.batchsize, shuffle=True, num_workers=2,
                                          drop_last=True)
trainloader2 = torch.utils.data.DataLoader(trainset, batch_size=args.batchsize, shuffle=True, num_workers=2)
valloader = torch.utils.data.DataLoader(valset, batch_size=args.batchsize, shuffle=True, num_workers=2, drop_last=True)
valtestloader = torch.utils.data.DataLoader(valtestset, batch_size=args.batchsize, shuffle=True, num_workers=2)
...
class Transf(nn.Module):
    def __init__(self, position, factor=1, base=[1, 0, 0, 0, 1, 0], distr='uniform', learn_factor='No'):
...
class Dataug(nn.Module):
    ...
    def __init__(self, transf=[], sampling=0.5):
        ...
        self.fc = sampling
        self.factor = nn.Parameter(1 * torch.ones(len(transf)))  # ,torch.ones(len(transf)))
        self.counter = 0
    ...
    def forward(self, x):
        ...
        if args.stoch:
            ...
            if args.smpuniform > 0 and (self.counter % args.smpuniform) == 0:
                self.qsmp = torch.ones(1, len(self.factor), device=torch.device('cuda')).softmax(1)
            else:
                self.qsmp = ((self.fc) * self.factor.view(1, -1) + (1 - self.fc) / len(self.factor)).softmax(1)
            if not (args.sametr):
                cat = Categorical(
                    probs=torch.ones((bs, len(self.factor))).cuda() * self.qsmp)  # *self.factor.view(1,-1))
                ...
                self.tsmp = torch.mm(torch.ones(bs, 1).cuda(), cat.sample().float().view(1, 1)).long()
                sel = self.tsmp[0]
                tr = self.transf[sel]
                if tr.distr == 'uniform':
                    theta = tr(usmp)
...
class Wloss(nn.Module):
    ...
    def forward(self, outputs, targets, tsmp=[], q=[]):
        if args.stoch:
            ...
            negreward = -outputs.log_softmax(1)[torch.arange(outputs.shape[0]).to(torch.long), targets]
        ...
        # sfs
            ...
            smax = self.factor.softmax(0)
            norm = smax
            fweight = torch.mm(norm.view(-1, 1), torch.ones(len(outputs) / len(self.factor)).cuda().view(1, -1)).view(
                -1).cuda()
            loss = (-outputs.log_softmax(1)[torch.arange(outputs.shape[0]).to(torch.long), targets] * fweight).mean()
...
class Prediction(nn.Module):
    ...
    def __init__(self, samples, classes):  # ,transf=[1,0,0,0,1,0]):
        ...
        self.factorpred = nn.Parameter(torch.ones(len(trainset) + len(valset), classes).cuda())

    def forward(self, outputs, smpid):
        loss = (-outputs.log_softmax(1)[torch.arange(outputs.shape[0])] * self.factorpred[smpid].softmax(1)).sum(
            1).mean()
        return loss
...
allnames = [x for x, y in net.named_parameters()] + [x for x, y in wcriterion.named_parameters()]
allparam = [y for x, y in net.named_parameters()] + [y for x, y in wcriterion.named_parameters()]
...
if len(alphaparam) > 0:
    if args.useadam:
        optimizeralphaval = optim.Adam(alphaparam, lr=args.lr * args.dlr)  # , momentum=0.0, weight_decay=0.0)#-5e-4)
    else:
        optimizeralphaval = optim.SGD(alphaparam, lr=args.lr * args.dlr, momentum=0.0, weight_decay=0.0)  # -5e-4)
...
def train(epoch, netparam, usetestforval, moms, log):
    ...
    for batch_idx, (inputs, targets, smpid) in enumerate(trainloader):
        ...
        fast_weights[pbuf + 1]['new'] = [x - real_lr * m for (x, m) in zip(weights, moms)]
        ...
        if len(alphaparam) > 0:
            if batch_idx % when_grad == 0:
                ...
                for l in range(len(fast_weights) - 1):
                    ...
                    [p.backward(curr_grad[idp], retain_graph=True) for idp, p in enumerate(fast_weights[-l - 2]['new'])]
    ...
    for batch_idx, (inputs, targets) in enumerate(loader):
        ...
        if args.stoch:
            if 1:
                ...
                loss = (-(net.module.smp1 + net.module.smp2 + net.module.smp3).mean(1) * outputs.softmax(1)[
                    torch.arange(outputs.shape[0]).to(torch.long), targets].log()).mean()
        else:
            loss = -outputs.softmax(1)[torch.arange(outputs.shape[0]).to(torch.long), targets].log().mean()
...
try:
    ...
    log = {'train_acc': [], 'val_acc': [], 'test_acc': [], "train": [], "val": [], "test": [], 'weights': [],
           'params': [], 'args': args}
