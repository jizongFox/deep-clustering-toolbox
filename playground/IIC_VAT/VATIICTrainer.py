...
class IMSATIICTrainer(IICMultiHeadTrainer):
    ...
    def __init__(self, model: Model, train_loader_A: DataLoader, train_loader_B: DataLoader, val_loader: DataLoader,
                 ...
                 head_control_params: dict = {}, use_sobel: bool = True, config: dict = None, adv_weight=0.1) -> None:
    ...
    def _training_report_dict(self):
        report_dict = {'train_MI_A': self.METERINTERFACE['train_head_A'].summary()['mean'],
    ...
    def _trainer_specific_loss(self, tf1_images, tf2_images, head_name):
        ...
        if self.adv_weight > 0:
            adv_loss, *_ = VATLoss_Multihead(xi=0.25, eps=1, prop_eps=0.1)(self.model.torchnet, tf1_images)
            self.METERINTERFACE[f'train_adv_{head_name}'].add(adv_loss.item())
        total_loss = iic_loss + self.adv_weight * adv_loss  # here the mi_batch_loss shoud be negative
        return total_loss
