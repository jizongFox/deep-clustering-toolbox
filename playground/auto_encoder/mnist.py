...
class MNISTTrainer(_Trainer):
    ...
    def __init__(self, model: Model,
                 ...
                 criterion: nn.Module,
                 max_epoch: int = 100,
                 save_dir: str = 'mnist', checkpoint_path: str = None, device='cpu', config: dict = None,
                 **kwargs) -> None:
...
class gradient_difference_loss(nn.Module):
    ...
    def forward(self, pred: torch.Tensor, gt: torch.Tensor):
        ...
        gdl_loss = self.abs_loss(orig_gradient_x, pred_gradient_x) + self.abs_loss(orig_gradient_y, pred_gradient_y)
...
model.optimizer = torch.optim.Adam(model.torchnet.parameters(), lr=1e-3,
                                   weight_decay=1e-5)
...
trainer = MNISTTrainer(model=model, train_loader=dataloader, val_loader=dataloader, criterion=nn.MSELoss(),
                       device='cuda', **config['Trainer'])
