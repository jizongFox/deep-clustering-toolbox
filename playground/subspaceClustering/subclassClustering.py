...
class SubSpaceClusteringMethod(_Method):
    ...
    def __init__(
        ...
        assert isinstance(device, torch.device), f"device should be torch.device, given {device}."
        self.lamda = float(lamda)
        self.alpha = float(alpha)
        self.device = device
        self.adj_matrix = torch.randn((num_samples, num_samples), dtype=torch.float32).to(self.device)
    ...
    def _diagnoal_remove(self, matrix):
        assert matrix.shape.__len__() == 2 and matrix.shape[0] == matrix.shape[
            1], f"check the matrix dimension, given {matrix.shape}"
    ...
    def set_input(self, imgs: Tensor, index: List[int], *args, **kwargs):
        ...
        assert imgs.shape[0] == len(index), f"imgs and index lengths should be the same, given len(imgs)=" \
    ...
    def _update_dictionary(self):
        ...
        current_adj_matrix_hat = self.current_adj_matrix - \
                                 self.lamda * self._representation.detach().mm(self._representation.detach().t()).mm(
            self.current_adj_matrix - torch.eye(len(self.current_adj_matrix)).to(self.device))
        current_adj_sign = current_adj_matrix_hat.sign()
        print(f"threshold = {self.lamda * (1 - self.alpha) / self.alpha}")
        new_current_adj = torch.max(
            current_adj_matrix_hat.__abs__() - self.lamda * (1 - self.alpha) / self.alpha,
...
class SubSpaceClusteringMethod2(SubSpaceClusteringMethod):
    ...
    def __init__(self, model: Model, alpha: float = 0.01, lamda: float = 0.0001, num_samples: int = 100,
                 device: torch.device = torch.device('cuda'), *args, **kwargs):
        super().__init__(model, alpha, lamda, num_samples, device, *args, **kwargs)
