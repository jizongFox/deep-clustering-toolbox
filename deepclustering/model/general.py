...
class Model(ABC):
    ...
    def _setup(self) -> Tuple[nn.Module, optim.SGD, torch.optim.lr_scheduler.LambdaLR]:
        if self.arch_dict is not None:
            ...
            warnings.warn(
                f"torchnet is a placeholder, override it later.", RuntimeWarning
            )
            self.arch_name = None
        ...
        if self.optim_dict is not None:
            ...
            warnings.warn(
                f"optimizer is a placeholder, override it later.", RuntimeWarning
            )
        ...
        if self.scheduler_dict is not None:
            ...
            scheduler: lr_scheduler.LambdaLR = getattr(
                lr_scheduler, self.scheduler_name
            )(optimizer, **self.scheduler_params)
        else:
            warnings.warn(
                f"scheduler is a placeholder, override it later.", RuntimeWarning
            )
