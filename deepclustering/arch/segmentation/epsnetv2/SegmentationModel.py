...
from .cnn_utils import PSPModule, CBR, BR, C


class EESPNet_Seg(nn.Module):
    def __init__(self,input_dim=3, num_classes=20, s=1, pretrained=None, gpus=1):
        super().__init__()
        ...
        self.project_l2 = CBR(self.net.level2_0.act.num_parameters + num_classes, num_classes, 1, 1)
        self.project_l1 = nn.Sequential(nn.Dropout2d(p=p),
                                        C(self.net.level1.act.num_parameters + num_classes, num_classes, 1, 1))
    ...
    def forward(self, input):
        ...
        if self.training:
            return F.interpolate(merge_l1, scale_factor=2, mode='bilinear',
                                 align_corners=True)  # , self.hierarchicalUpsample(proj_merge_l3_bef_act)
        else:
...
if __name__ == '__main__':
    ...
    net = EESPNet_Seg(num_classes=20, s=2).cuda()
    out_x_8 = net(input)
    print(out_x_8.size())
