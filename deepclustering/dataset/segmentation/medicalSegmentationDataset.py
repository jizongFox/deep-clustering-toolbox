...
class MedicalImageSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        assert reduce(and_, [isinstance(s, str) for s in subfolders]), \
        ...
        self.transform: SequentialWrapper = transforms if transforms else \
            SequentialWrapper(
    ...
    def make_dataset(cls, root: str, mode: str, subfolders: List[str], verbose=True):
        ...
        for subfolder in subfolders:
            assert Path(root, mode, subfolder).exists(), os.path.join(root, mode, subfolder)
...
class MedicalImageSegmentationDatasetWithMetaInfo(MedicalImageSegmentationDataset):
    ...
    def __init__(self, root_dir: str, mode: str, subfolders: List[str], transforms=None, verbose=True,
                 metainfo_generator: Callable = None) -> None:
