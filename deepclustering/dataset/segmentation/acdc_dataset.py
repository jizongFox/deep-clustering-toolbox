...
from deepclustering.dataset.segmentation._medicalSegmentationDataset import MedicalImageSegmentationDataset
from deepclustering.dataset.segmentation._patient_sampler import SubMedicalDatasetBasedOnIndex
...
class ACDCDataset(MedicalImageSegmentationDataset):
    ...
    def __init__(
            self, root_dir: str, mode: str, subfolders: List[str], transforms: SequentialWrapper = None,
            verbose=True
    ) -> None:
        ...
        super().__init__(
            os.path.join(root_dir, self.folder_name), mode, subfolders, transforms, "patient\d+_\d+_\d+", verbose
        )


class ACDCSemiInterface(MedicalDatasetSemiInterface):

    def __init__(self, labeled_data_ratio: float = 0.2, unlabeled_data_ratio: float = 0.8, seed: int = 0,
                 verbose: bool = True) -> None:
        super().__init__(ACDCDataset, DATA_PATH, labeled_data_ratio, unlabeled_data_ratio, seed, verbose)
    ...
    def _create_semi_supervised_datasets(
        ...
        train_set = self.DataClass(root_dir=self.root_dir, mode="train", subfolders=["img", "gt"], transforms=None,
                                   verbose=self.verbose)
        val_set = self.DataClass(root_dir=self.root_dir, mode="val", subfolders=["img", "gt"], transforms=None,
                                 verbose=self.verbose)
        ...
        assert labeled_set.filenames["img"].__len__() + unlabeled_set.filenames["img"].__len__() == train_set.filenames[
            "img"].__len__(), "wrong on labeled/unlabeled split."
        del train_set
