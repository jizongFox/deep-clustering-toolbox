...
class Volume(object):
    ...
    def __init__(self, img_folder: str, mask_folder_list: List[str], group_pattern=r'patient\d+_\d+',
                 img_extension: str = 'png', crop: int = 0, mapping=None, resize=None) -> None:
        super().__init__()
        ...
        if len(mapping) > 0:
            ...
            if len(mask_folder_list) == 0:
                raise RuntimeError(f"No mask is given, you do not need to have mapping, given {mapping}")
            # case 2: all gt_folder has the same mapping
                ...
                self.mapping_modules: Dict[str, Callable[[np.ndarray], np.ndarray]] = {
                    f"{str(m)}": np.vectorize(lambda x: mapping[0].get(x)) for m in
                    self.mask_folder_list}
            elif len(mapping) == len(mask_folder_list) and len(mask_folder_list) >= 2:
                print(f"Found {len(mapping)} mapping: {mapping}, applying to {len(mask_folder_list)} gt_masks")
                self.mapping_modules: Dict[str, Callable[[np.ndarray], np.ndarray]] = {
                    f"{str(m)}": Vectorize(mapping_) for m, mapping_ in zip(self.mask_folder_list, mapping)}
            else:
                raise NotImplementedError("mapping logic is wrong.")
        else:
            # no mapping is needed, use identical mapping
            self.mapping_modules: Dict[str, Callable[[np.ndarray], np.ndarray]] = {"original image": identical}
            self.mapping_modules.update({f"{m}": identical for m in self.mask_folder_list})
    ...
    def _preload_subjects(self, batch_image_path: List[Path], batch_mask_paths_dict: Dict[str, List[Path]],
                          mapping_module) \
            -> Tuple[np.ndarray, Dict[str, np.ndarray]]:
        img_source: np.ndarray = np.stack([np.array(Image.open(str(x)).convert('P')) for x in batch_image_path],
                                          axis=0)
        ...
        if self.resize:
            img_source, mask_source_dict = self.optional_resize(img_source, mask_source_dict, self.resize)

        return img_source, mask_source_dict

    @staticmethod
    def optional_resize(img_source_np_tensor: np.ndarray, mask_source_np_dict: Dict[str, np.ndarray],
                        resize=[256, 256]):
        ...
        # bi-linear resize
        resized_img_tensor = img_source_np_tensor if not _if_need_resized(img_source_np_tensor) else \
            np.stack([resize_func(img_slice, output_shape=resize, order=1, preserve_range=True) for img_slice in
                      img_source_np_tensor],
                     axis=0)
        # Nearest-neighbor
        resized_mask_source_np_dict = {}
        for k, v in mask_source_np_dict.items():
            resized_mask_source_np_dict[k] = v if not _if_need_resized(v) else \
                np.stack(
                    [resize_func(mask_slice, output_shape=resize, order=0, preserve_range=True, anti_aliasing=False) for
                     mask_slice in v],
                    axis=0)
