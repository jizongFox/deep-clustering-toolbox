...
class _DataLoaderIter(object):
    ...
    def _get_batch(self):
        ...
        if self.timeout > 0:
            ...
            while True:
                success, data = self._try_get_batch() # just to checkout data from Queue.
                if success:
                    return data
    ...
    def __next__(self):
        ...
        while True:
            ...
            idx, batch = self._get_batch() # Fetches data from `self.data_queue`
            self.batches_outstanding -= 1
            if idx != self.rcvd_idx:
                # store out-of-order samples
                self.reorder_dict[idx] = batch
                continue
            return self._process_next_batch(batch) # put the indexes of the next batch to Queue.
